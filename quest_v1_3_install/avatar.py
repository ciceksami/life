...
class Avatar(Module):
                ...
                for item in const.room_items:
                    ...
                    for i in range(1, 6):
                        self.server.modules["frn"].add_item(item, str(i),
                                                            client.uid)
                quest = self.server.modules.get("qs")
                if quest:
                    quest.record_event(client.uid, "create_avatar", 1)
            else:
    ...
    def buy_cloth(self, cloth, client):
        ...
        try:
            quest = self.server.modules.get("qs")
            if quest:
                quest.record_event(client.uid, "purchase", 1)
        except Exception:
            logging.exception(
                "Quest purchase event error | uid=%s | source=avatar.buy_cloth",
                client.uid
            )
    ...
    def buy_clothes_suit(self, tpid, client):
        ...
        if to_buy:
            try:
                ...
                logging.exception(
                    "Quest purchase event error | uid=%s | source=avatar.buy_clothes_suit",
                    client.uid
                )
    ...
    def buy_colored_clothes(self, clothes, client):
        ...
        try:
            ...
            logging.exception(
                "Quest purchase event error | uid=%s | source=avatar.buy_colored_clothes",
                client.uid
            )
