...
class QuestSystem(Module):
    ...
    QUESTS = {
        ...
        "daily_play_time": {
            "type": "daily", "event": "play_minute", "target": 30,
            "reward": economy.DAILY_QUEST_REWARDS["play_time"],
            "title": "30 dakika oyunda kal"
        },
        "daily_social": {
        ...
        # Weekly
        "weekly_active": {
            "type": "weekly", "event": "play_minute", "target": 180,
            "reward": economy.WEEKLY_QUEST_REWARDS["active_player"],
            "title": "Haftada 180 dakika oyunda kal"
        },
        "weekly_social": {
    ...
    def _progress_key(self, uid, quest_id, quest_type):
        return f"uid:{uid}:quest:{self._period(quest_type)}:{quest_id}:progress"

    def _claimed_key(self, uid, quest_id, quest_type):
        return f"uid:{uid}:quest:{self._period(quest_type)}:{quest_id}:claimed"
    ...
    def claim_reward(self, uid, quest_id):
        ...
        if int(redis.get(progress_key) or 0) < int(quest["target"]):
        ...
        return {"ok": True, "id": quest_id, "reward": reward}
