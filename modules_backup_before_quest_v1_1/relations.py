...
class Relations(Module):
    ...
    def _create_relation(self, link, relation):
        ...
        if quest:
            for quest_uid in link.split(":"):
                quest.record_event(quest_uid, "social", 1)
        for uid in link.split(":"):
    ...
    def add_progress(self, action, link):
        if action not in self.progresses:
            return
        value = self.progresses[action]
        s_raw = self.server.redis.get(f"rl:{link}:s")
        p_raw = self.server.redis.get(f"rl:{link}:p")
        if s_raw is None or p_raw is None:
            return
        s = int(s_raw)
        p = int(p_raw)
