...
class Passport(Module):
    ...
    def passport(self, msg, client):
        ...
        for item in self.server.achievements:
            progress = int(self.server.redis.get(f"uid:{msg[2]['uid']}:ach:{item}:p") or 0)
            level = int(self.server.redis.get(f"uid:{msg[2]['uid']}:ach:{item}:l") or 3)
            ac[item] = {"p": progress, "nWct": 0, "l": level, "aId": item}
        tr = {}
        for item in self.server.trophies:
            rating = int(self.server.redis.get(f"uid:{msg[2]['uid']}:tr:{item}:rt") or 0)
            code = int(self.server.redis.get(f"uid:{msg[2]['uid']}:tr:{item}:cd") or 0)
            tr[item] = {"trrt": rating, "trcd": code, "trid": item}
